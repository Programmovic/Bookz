...
function Shop(props) {
  ...
  useEffect(() => {
    ...
    if ((JSON.stringify(productsAvailableList) === JSON.stringify([]))
      ...
      try {
        (async () => {
          const productsAvailableData = await axios.get('/books')
  ...
  useEffect(() => {
    ...
    if (id) {
      ...
      async function getUpdatedWishlistAndCart() {
        try {
          const updatedUserInfo = await axios.get(`/user/${id}`);
          if (updatedUserInfo.data.status === "ok") {
            dispatchUserWishlist({ type: "UPDATE_USER_WISHLIST", payload: updatedUserInfo.data.wishlist })
            dispatchUserCart({ type: "UPDATE_USER_CART", payload: updatedUserInfo.data.cart })
